...
public:
   ...
   /*
	* 下面的函数想要根据我们默认的误算率(false positive)来
	* 找出最优的hash函数个数和存储的数据的数量，并评估插入元素的数量
	*/
...
protected:
   ...
   void generate_unique_salt()
   {
      ...
      */
      ...
      {
		  /*copy(first_itr, last_itr, result) 
		   * 将[first_itr, last_itr)的元素拷贝到result中
		   * back_inserter(Container& x) 未容器返回一个尾指针
		   */
   ...
   unsigned int            salt_count_; //hash函数个数
